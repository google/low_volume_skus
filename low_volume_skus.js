 ...
 * Some other variables may be modified by demand, following guideline in comments.
...
var SPREADSHEET_URL = 'https://docs.google.com/spreadsheets/d/1ipSoiNg3vANswS8U9_JD5KAZP1EhFVyNnN1C63C31l0/edit';
...
var FILTER_RAMPED_UP = 'metrics.clicks > ' + THRESHOLD + ' AND segments.product_custom_attribute' + CUSTOM_LABEL_NR + ' = "' + LABEL_LOW + '" ';
...
function main() {
  var productsNoClicks = getFilteredShoppingProducts(FILTER_NO_CLICKS, checkLabel=false);
  var productsRampedUp = getFilteredShoppingProducts(FILTER_RAMPED_UP, checkLabel=true);
...
function getFilteredShoppingProducts(filters,checkLabel) {
  ...
  var labelField = ''
  if (checkLabel) {labelField = 'segments.product_custom_attribute' + CUSTOM_LABEL_NR +', '};
  ...
  while (rows.hasNext()) {
    ...
    var productCustomlabel = 'Test';
    
    // Label product as low volume, if no clicks (in last 30 days defined at the top).
    if (clicks < THRESHOLD ) {
      products.push([productId, LABEL_LOW]);
      count += 1;
      
    // Label product as ramped up, if it surpasses expected threshold.
    } else if (productCustomlabel == LABEL_LOW && clicks > parseInt(THRESHOLD)) {
